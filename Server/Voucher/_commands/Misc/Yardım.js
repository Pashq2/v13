  ...
  onLoad: function (client) {
    client.on('interactionCreate', async i => {
        ...
        if (i.customId === 'acar_yardim') {
            if(i.values == "talent") {
                i.reply({embeds: [new genEmbed().setDescription(`${Data ?  Data.talentPerms ? Data.talentPerms.map(x => `\`${sistem.botSettings.Prefixs[0] + x.Commands + " <@pashq/ID>"}\``).join("\n") : '' : ''}`)], ephemeral: true})
           } else {
  ...
  onRequest: async function (client, message, args) {
    ...
    collector.on("collect", i => {
        if(i.customId == "yardimmenusu") {
           if(i.values == "talent") {
                acar.edit({embeds: [new genEmbed().setDescription(`${Data ?  Data.talentPerms ? Data.talentPerms.map(x => `\`${sistem.botSettings.Prefixs[0] + x.Commands + " <@pashq/ID>"}\``).join("\n") : '' : ''}`)]})
           } else {
